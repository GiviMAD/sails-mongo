...
Collection.prototype.find = function find(criteria, cb, meta) {
  var self = this,
      query;
  ...
  var cursor = collection.find(where, query.select, queryOptions);

  if (meta && meta.collation) {
    cursor = cursor.collation(meta.collation);
  }
 ...
 * @api public
 */
Collection.prototype.stream = function find(criteria, stream, meta) {
  var self = this,
    query;

  if (meta && meta.collation) {
    return stream.end(new Error('This operation does not support collation.'));
  }
...
Collection.prototype.insert = function insert(values, cb, meta) {
  var self = this;

  if (meta && meta.collation) {
    return cb(new Error('This operation does not support collation.'));
  }
...
Collection.prototype.update = function update(criteria, values, cb, meta) {
  var self = this,
      query;
  ...
  var collection = this.connection.db.collection(self.identity, { readPreference: ReadPreference.PRIMARY });
  ...
  var cursor = collection.find(query.criteria.where, {_id: 1});
  ...
  cursor.toArray(function(err, records) {
    ...
    // Update the records
    collection.update({ _id: { '$in': updatedRecords }}, { '$set': values }, { multi: true }, function(err, result) {
      if(err) return cb(err);
...
Collection.prototype.destroy = function destroy(criteria, cb, meta) {
  var self = this;
  var query;
...
Collection.prototype.count = function count(criteria, cb, meta) {
  var query;
